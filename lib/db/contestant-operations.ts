...
export async function getAllContestants(): Promise<Contestant[]> {
  return await db.select().from(contestants).orderBy(asc(contestants.serial_number));
}
  ...
  return await db.select()
    ...
    .where(
      or(
        ...
        like(contestants.contact, searchTerm)
      )
    )
...
export async function getContestantsByStatus(status: 'Competing' | 'Eliminated'): Promise<Contestant[]> {
  return await db.select()
    .from(contestants)
...
export async function getTopPerformers(): Promise<Contestant[]> {
  return await db.select()
    ...
    .orderBy(asc(contestants.serial_number))
    .limit(5);
}
