...
export default async function RecordingSchedulePage() {
  const user = await getCurrentUser();
  const episodes = await getAllEpisodes();

  // Sort episodes by recording start date
  const recordingEpisodes = episodes
    .filter(episode => {
      const status = getEpisodeStatus(episode);
      return status !== 'Aired';
    })
    .sort((a, b) => new Date(a.record_start).getTime() - new Date(b.record_start).getTime());

  // Group by date
  const episodesByDate = recordingEpisodes.reduce((acc, episode) => {
    const date = formatPKTDate(episode.record_start);
    if (!acc[date]) {
      acc[date] = [];
    }
    acc[date].push(episode);
    return acc;
  }, {} as Record<string, typeof recordingEpisodes>);
    ...
    <Layout user={user}>
      <div className="space-y-6">
        ...
        <div className="text-center">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Recording Schedule</h1>
          <p className="text-slate-600">Chronological schedule of all recording sessions</p>
        </div>
        ...
        {Object.keys(episodesByDate).length > 0 ? (
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-slate-900 mb-2">No Recording Schedule</h3>
              <p className="text-slate-600">No recording sessions are currently scheduled.</p>
            </CardContent>
          </Card>
        )}
      </div>
