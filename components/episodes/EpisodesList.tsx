          ...
          {filteredEpisodes.map((episode) => {
            ...
            return (
              <div key={episode.episode_id}>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center">
                            <span className="text-white font-bold text-xs">E{episode.episode_no}</span>
                          </div>
                          <div>
                            <div className="flex items-center space-x-2">
                              <span className="font-semibold text-slate-900 text-sm">Episode {episode.episode_no}</span>
                              <StatusChip status={status} />
                            </div>
                            <div className="text-xs text-slate-600 mt-1">
                              {episode.phase} • {episode.city}
                            </div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-xs text-slate-500">
                            {formatPKTDate(episode.air_start, 'dd MMM')}
                          </div>
                          <div className="text-xs text-slate-400">
                            {formatPKTDate(episode.air_start, 'HH:mm')}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <Card className="hidden md:block hover:shadow-xl transition-all duration-200 border border-slate-200 bg-white shadow-sm">
                  <CardContent className="p-6">
